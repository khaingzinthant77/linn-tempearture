...
@section('content')
 <div class="container" >
        <form action="{{route('training_emp.update',$training_employees->id)}}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="col-md-5">
            ...
            <select class="form-control" name="training_id" id="training_id">
              ...
              <option  value="{{$training->id}}" {{ (old('training_id',$training_employees->training_id)==$training->id)?'selected':'' }}>{{$training->name}}</option>
              @endforeach
           </select>
        ...
        <div class="col-md-5">
            ...
            <select class="livesearch form-control" name="emp_id">
                 @foreach ($employees as $employee )
                  <option  value="{{$employee->id}}" {{ (old('emp_id',$training_employees->emp_id)==$employee->id)?'selected':'' }}>{{$employee->name}}</option>
                @endforeach
            </select>
